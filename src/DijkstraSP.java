...
public class DijkstraSP {
     ...
     */
    public DijkstraSP(EdgeWeightedDigraph G, int s,int destination,int time) {
        ...
        while (!pq.isEmpty() && pq.minIndex() != destination) {
            v = pq.delMin();
    ...
    private void relax(DirectedEdge e,int time) {
        int v = e.from(), w = e.to();
        if (distTo[w] > distTo[v] + e.weight()) {
            distTo[w] = distTo[v] + e.weight();
            time = (time+((int)distTo[v]));
            if (((time >= 420 && time <= 480)  || (time >= 720 && time <= 780) || (time >= 1020 && time <= 1080 ))&& e.rushHour() == true){
                distTo[w] *= 1.9;
            }
            edgeTo[w] = e;
